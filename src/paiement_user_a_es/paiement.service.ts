...
export class PaiementService {
  ...
  try {
    ...
    await manager.query(
      ...
      // Notifications
      const [etablissementNotif] = await manager.query(
        `SELECT fcm_token FROM user_etablissement_sante WHERE id_user_etablissement_sante = $1`,
        [compteEtablissement.id_user_etablissement_sante]
      );

      if (etablissementNotif?.fcm_token) {
        await this.notificationService.sendToToken(
          etablissementNotif.fcm_token,
          '💰 Paiement reçu',
          `Vous avez reçu ${montantFinal} XOF via QR Code`
        );
      }

      await manager.query(
        `INSERT INTO notification_transaction (id_transaction, identif_transaction, type_notification, contenu, montant, id_user_etablissement_sante, statut, is_lu)
    ...
    // ✅ d. Insérer dans transactions_frais avec le bon id_transaction
      ...
      if (etablissementNotif?.fcm_token) {
        await this.notificationService.sendToToken(
          etablissementNotif.fcm_token,
          '💰 Paiement reçu',
          `Vous avez reçu ${montantFinal} XOF`
        );
      }
