...
return {
  ...
  // PARTIE DES AGENTS HOSTOPAY POUR LES RECHARGEMENTS ET RETRAITS DES UTILISATEURS
...
async crediterUtilisateur(id_user: string, montant: number, idAdmin: number) {
  ...
  try {
    ...
    [compte] = await this.dataSource.query(
    ...
    [admin] = await this.dataSource.query(
    ...
    // Enregistrement de l’opération échouée dans admin_rechargements
    try {
...
async crediterEtablissement(idEtab: number, montant: number, idAdmin: number) {
  ...
  try {
    ...
    [compte] = await this.dataSource.query(
      `SELECT * FROM compte WHERE id_user_etablissement_sante = $1 AND statut = 'actif' LIMIT 1`,
      [idEtab],
    );
...
async retirerEtablissement(idEtab: number, montant: number, idAdmin: number) {
  ...
  let compte: any = null;
  let admin: any = null;
  let ancienSolde: number | null = null;
  let nouveauSoldeUser: number | null = null;
  ...
  try {
    ...
    await this.dataSource.transaction(async manager => {
      ...
      await manager.query(
        `INSERT INTO admin_rechargements (id_admin, cible_type, cible_id, identifiant, montant, ancien_solde, nouveau_solde, statut, erreur_operations, type_operation)
         VALUES ($1, 'user', $2, $3, $4, $5, $6, $7, $8, $9)`,
        [idAdmin, user.id_user, identifiant, montant, ancienSolde, nouveauSoldeUser, 'reussie', null, 'dépot',]
      );
    });
...
async rechargerEtablissement(identifiant: string, montant: number, idAdmin: number) {
  ...
  let admin: any = null;
  let ancienSolde: number | null = null;
  let nouveauSoldeEtab: number | null = null;
